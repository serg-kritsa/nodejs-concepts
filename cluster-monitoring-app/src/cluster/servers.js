...
const port = 8181;
const num_processes = require('os').cpus().length; // number of logical cores
...
if (cluster.isMaster) {
	...
	const worker_index = function(ip, len) {
        // const hashAsUInt32Low = highwayhash.asUInt32Low(key, ip);
        // // const hash = createHash('sha256').update(ip).digest('hex');
		// // console.log(`${hash.update(ip).digest('hex')}`);
		// return hashAsUInt32Low % len;
	...
	// Don't expose our internal server to the outside world. master is listening all requests
	...
	// redis-cli monitor
	// io.adapter(io_redis({ host: 'redis', port: 6379 })); // FOR DOCKER
	...
	// Listen to messages sent from the master via worker.send(.., ..). Ignore everything else.
